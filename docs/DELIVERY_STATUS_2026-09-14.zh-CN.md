...
- 本地结果：`npm test` 完成生产 Bundle 构建并执行 206 项测试，205 项通过，仅跳过
  需要独立 PostgreSQL 的 Opt-in 套件，0 项失败；
- 远端结果：[GitHub Actions run 34813619001](https://github.com/yyzxide/runguild/actions/runs/34813619001) 通过；
...
| R4：未知价格被聚合成 0 | 代码缺陷已关闭；对照证据待补 | `e1fc591` 将未知成本保留为 `null`，展示价格覆盖率，并把少于三组完整配对标为探索性。拟定的 18 个 Live-model Trial 尚未运行。 |
...
- 在受保护验收与 Bubblewrap 下重新运行一个有界目标，并包含一次有意错误产物被拒绝；
- 三类任务、Single/Multi-Agent 配对、每类每种策略重复三次：从冻结基线新建 18 个
  Live-model Trial；
- 上述证据完成后录制一段短而可复现的 Walkthrough。

18 Trial 只是工程小样本，不代表统计显著。历史 Trial 仍是有价值的失败与恢复证据，
但不会与平台修复后的结果混算。
