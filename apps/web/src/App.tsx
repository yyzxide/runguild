...
import { experiments, traceEvents, type EvidenceFact, type MissionTask, type TaskStatus } from './data'
...
function TraceView() {
  const [selected, setSelected] = useState<(typeof traceEvents)[number]>(traceEvents[5])
  return <><section className="page-heading"><div><div className="breadcrumb"><span>运行记录</span><i>/</i><code>示例 run_7f2…c81</code></div><h1>构建 Agent Run · 第 01 次尝试</h1><p>Ivo 执行任务-02 的完整记录，包括模型实际可见的上下文。当前页面展示的是示例账本。</p></div><div className="page-actions"><StatusPill tone="active"><span className="pulse-dot" />执行中</StatusPill></div></section><section className="trace-contract"><div><span>Agent</span><strong>Ivo · 构建者</strong></div><div><span>租约</span><code>剩余 44 秒</code></div><div><span>模型</span><code>gpt-5.2-codex</code></div><div><span>上下文</span><code>42,810 / 65,536</code></div><div><span>成本</span><code>预计 $0.82</code></div></section><div className="trace-workspace"><section className="trace-list-panel"><div className="panel-heading"><div><span className="micro-label">有序持久化事件</span><h2>执行瀑布流</h2></div><code>09:44:32 → 现在</code></div><div className="trace-list">{traceEvents.map((event, index) => <button key={event.time} className={`trace-event trace-event--${event.state}${selected.time === event.time ? ' is-selected' : ''}`} onClick={() => setSelected(event)}><span className="trace-event__index">{String(index + 1).padStart(2, '0')}</span><code className="trace-event__time">{event.time}</code><span className={`trace-event__type trace-event__type--${event.type}`}>{event.type}</span><div><strong>{event.title}</strong><p>{event.detail}</p></div><code className="trace-event__duration">{event.duration}</code></button>)}</div></section><aside className="trace-detail"><span className="micro-label">选中事件</span><h2>{selected.title}</h2><p>{selected.detail}</p><dl><div><dt>发生时间</dt><dd><code>{selected.time}</code></dd></div><div><dt>持续时间</dt><dd><code>{selected.duration}</code></dd></div><div><dt>关联 ID</dt><dd><code>mission_24f…109</code></dd></div><div><dt>内容哈希</dt><dd><code>c3a5d9…2e10</code></dd></div></dl><div className="context-meter"><div><span>模型可见上下文</span><code>65.3%</code></div><i><b style={{ width: '65.3%' }} /></i><small>保留冻结前缀和原子工具交换。</small></div><button className="quiet-action"><FileStack size={15} />查看原始账本行</button></aside></div></>
}
...
export function App() {
  ...
  const content = useMemo(() => {
    ...
    if (view === 'trace') return <TraceView />
